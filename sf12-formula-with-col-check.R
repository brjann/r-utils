...
#' @export 
sf12_calc <- function( X = NULL ) {

  if((!(is.data.frame(X) | is.matrix(X))) | (ncol(X)!=12) )
    stop("X must be a data.frame (or matrix) with 12 columns")
  
  expected <- c("gh1", "pf02", "pf04", "rp2", "rp3", "re2", "re3", "bp2",
    "mh3", "vt2", "mh4", "sf2" )
  for(i in 1:length(expected)){
    check <- names(X)[i]
    exp <- expected[i]
    match <- grep(paste0(".*", exp, ".*"), check)
    if(length(match) != 1){
      stop("Column mismatch. Column ", check, " should include ", exp)
    }
  }
